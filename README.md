...
如需按路径或状态码判断（例如只要 `200–299`），再到「监视器」创建自定义检查，并在池中改绑。需要虚拟主机时，在该监视器中填写对应站点的 `Host`，也可配置端口、请求方法、附加请求头和跳转策略。
...
- `workers/health`：内部健康模块，每分钟由同一个 Worker 的 Cron 唤醒。默认连续失败 2 次 Down；可达性检查一次成功即可恢复。
