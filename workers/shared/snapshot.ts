...
type OriginRow = { id: string; name: string; address: string; connection_host: string | null; region: string; latitude: number; longitude: number; weight: number }
type MonitorRow = { id: string; name: string; type: 'HTTP' | 'HTTPS' | 'TCP'; method: 'GET' | 'HEAD'; path: string; port: number | null; interval_seconds: number; timeout_seconds: number; expected_codes: string; consecutive_fails: number; consecutive_successes: number; headers_json: string; follow_redirects: number }
...
type LoadBalancerRow = { id: string; hostname: string; origin_host: string | null; site: string; steering: SteeringPolicy; session_affinity: number; affinity_ttl_seconds: number; proximity_buffer: number }
...
export async function buildSnapshot(db: D1Database, version: number): Promise<ConfigSnapshot> {
  const [originsResult, monitorsResult, poolsResult, poolOriginsResult, loadBalancersResult, loadBalancerPoolsResult, healthResult, sampleRateResult] = await Promise.all([
    db.prepare('SELECT id, name, address, connection_host, region, latitude, longitude, weight FROM origins WHERE enabled = 1 ORDER BY created_at').all<OriginRow>(),
    db.prepare('SELECT id, name, type, method, path, port, interval_seconds, timeout_seconds, expected_codes, consecutive_fails, consecutive_successes, headers_json, follow_redirects FROM monitors ORDER BY created_at').all<MonitorRow>(),
    ...
    db.prepare('SELECT id, hostname, origin_host, site, steering, session_affinity, affinity_ttl_seconds, proximity_buffer FROM load_balancers WHERE enabled = 1 ORDER BY created_at').all<LoadBalancerRow>(),
  ...
  for (const row of resultRows(originsResult)) snapshot.origins[row.id] = { id: row.id, name: row.name, address: row.address, connectionHost: row.connection_host, region: row.region, latitude: row.latitude, longitude: row.longitude, weight: row.weight }
  for (const row of resultRows(monitorsResult)) snapshot.monitors[row.id] = { id: row.id, name: row.name, type: row.type, method: row.method, path: row.path, port: row.port, intervalSeconds: row.interval_seconds, timeoutSeconds: row.timeout_seconds, expectedCodes: row.expected_codes, consecutiveFails: row.consecutive_fails, consecutiveSuccesses: row.consecutive_successes, headers: parseHeaders(row.headers_json), followRedirects: Boolean(row.follow_redirects) }
  ...
  for (const row of resultRows(loadBalancersResult)) snapshot.loadBalancers[row.hostname] = { id: row.id, hostname: row.hostname, originHost: row.origin_host, site: row.site, steering: row.steering, sessionAffinity: Boolean(row.session_affinity), affinityTtlSeconds: row.affinity_ttl_seconds, proximityBuffer: row.proximity_buffer, pools: [] }
